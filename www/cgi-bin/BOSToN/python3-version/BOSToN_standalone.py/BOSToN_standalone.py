...
def DefCentromeres(): # retrieve the centromere position
    f = open("centromere2.txt", "r") # currently a static file that needs to be updated with each build
    centromeres = np.loadtxt("centromere2.txt", usecols = (2,3))
    f.close()
    centromeres = centromeres.tolist()
    return centromeres

def DefBands(): #defines breakpoints cytogenetic bands
    f = open("Mbn.csv", "rb") #currently a static file that needs to be updated with each build
    reader = csv.reader(f)
    bands = []
    for row in reader:
	bands.append(row)
...
def GetXmlBlastRecords(fasta_file): #module to handle I/O to NCBI Blast and return XML output
    ...
    XmlBlastRecords = "blastout.xml"
    blastn_cline = NcbiblastnCommandline(cmd="blastn", query=fasta_input_file, db="human_genomic.02", outfmt=5, out=XmlBlastRecords, task="megablast")
...
def GetArm(blast_output): #determines chromosome arm of breakpoints (i.e. p or q)
	blast_arm = list(range(0, len(blast_output)))
...
def BPEssentials(blast_output, blast_arm, QxyD, BANDxy, moredata1, M1 = "Nothing"): #creates a list for each breakpoint end
	blast_output2 = list(map(list, blast_output))
	blast_arm2 = list(map(list, blast_arm))
...
def read_fasta_input():
    ...
    if arg_len < 2:
        print('Usage: python', sys.argv[0], '[path_to_fasta_file]')
        sys.exit()
