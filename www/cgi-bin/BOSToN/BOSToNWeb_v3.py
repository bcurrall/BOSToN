...
def GetXmlBlastRecords(fasta_file): #module to handle I/O to NCBI Blast and return XML output
    ...
    if struct.calcsize("P") * 8 == 32:
        blastn_cline = NcbiblastnCommandline(cmd="blastn_32", query=fasta_input_file, db="human_genomic.02", outfmt=5, evalue=0.001, out=XmlBlastRecords, task="megablast", max_target_seqs=25, best_hit_overhang=0.1)
    elif struct.calcsize("P") * 8 == 64:
        db_file = os.path.join(GetAppDir(),"human_genomic.02")
        blastcmd = os.path.join(GetAppDir(), "blastn_64")
        blastn_cline = NcbiblastnCommandline(cmd=blastcmd, query=fasta_input_file, db="human_genomic.02", outfmt=5, evalue=0.001, out=XmlBlastRecords, task="megablast", max_target_seqs=25, best_hit_overhang=0.1)
    blastn_cline()
